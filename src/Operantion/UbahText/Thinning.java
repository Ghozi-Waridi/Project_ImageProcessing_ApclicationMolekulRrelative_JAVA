...
public class Thinning {
    ...
    public  BufferedImage fixLines(BufferedImage img, int distanceThreshold) {
        int lebar = img.getWidth();
        int tinggi = img.getHeight();
        ...
        for(int y = 0; y < tinggi; y++){
            for(int x = 0; x < lebar; x++){
                gambar.setRGB(x, y, img.getRGB(x,y));
            }
        }

        for (int y = 0; y < tinggi; y++) {
            for (int x = 0; x < lebar; x++) {
                int p = new Color(img.getRGB(x,y)).getRed();
                if (p == 255) {
                    for (int di = -distanceThreshold; di <= distanceThreshold; di++) {
                        for (int dj = -distanceThreshold; dj <= distanceThreshold; dj++) {
                            int ni = x + di;
                            int nj = y + dj;
                            int pixel = new Color(img.getRGB(ni,nj)).getRed();
